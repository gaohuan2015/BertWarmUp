...
def tensorFromSentence(lang, sentence):
    ...
    indexes.append(SD.EOS_token)
    return torch.tensor(indexes, dtype=torch.long, device=device).view(-1, 1)


def tensorsFromPair(pair):
    input_tensor = tensorFromSentence(SD.input_lang, pair[0])
    target_tensor = tensorFromSentence(SD.output_lang, pair[1])
    return (input_tensor, target_tensor)

teacher_forcing_ratio = 0.5


def train(input_tensor, target_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, max_length=SD.MAX_LENGTH):
    ...
    decoder_input = torch.tensor([[SD.SOS_token]], device=device)
    ...
    if use_teacher_forcing:
        ...
        for di in range(target_length):
            ...
            loss += criterion(decoder_output, target_tensor[di])
            if decoder_input.item() == SD.EOS_token:
                break
...
def trainIters(encoder, decoder, n_iters, print_every=1000, plot_every=100, learning_rate=0.01):
    ...
    training_pairs = [tensorsFromPair(random.choice(SD.pairs))
...
def evaluate(encoder, decoder, sentence, max_length=SD.MAX_LENGTH):
    with torch.no_grad():
        input_tensor = tensorFromSentence(SD.input_lang, sentence)
        input_length = input_tensor.size()[0]
        encoder_hidden = encoder.initHidden()
