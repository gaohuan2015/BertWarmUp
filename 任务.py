...
if test==0:
    ...
    torch.save(EncoderRNN, 'EncoderRNN.pth')
    torch.save(DecoderRNN, 'DecoderRNN.pth')
    torch.save(AttnDecoderRNN, 'AttnDecoderRNN.pth')
    ...
    print(len(pairs))
    # print(random.choice(pairs))
    EncoderRNN = torch.load('EncoderRNN.pth')
    DecoderRNN = torch.load('DecoderRNN.pth')
    AttnDecoderRNN = torch.load('AttnDecoderRNN.pth')
    encoder1 = EncoderRNN(input_lang.n_words, hidden_size).to(device)
    attn_decoder1 = AttnDecoderRNN(hidden_size, output_lang.n_words, dropout_p=0.1).to(device)
