...
# 加载数据集
corpus = Corpus()
ids = corpus.get_data('train.en', batch_size)
...
class RNNLM(nn.Module): #RNNLM类继承nn.Module类
    def __init__(self, vocab_size, embed_size, hidden_size, num_layers):
        ...
        self.embed = nn.Embedding(vocab_size, embed_size)   #嵌入层 one-hot形式(vocab_size,1) -> (embed_size,1)
        self.lstm = nn.LSTM(embed_size, hidden_size, num_layers, batch_first=True)   #LSTM单元/循环单元
        self.linear = nn.Linear(hidden_size, vocab_size)   #输出层的全联接操作
...
optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)  #使用Adam优化方法 最小化损失 优化更新模型参数
...
with torch.no_grad():
    with open('sample.en', 'w') as f:
        ...
        for i in range(num_samples):
            ...
            if (i+1) % 100 == 0:
                print('generated [{}/{}] words，save {}'.format(i+1, num_samples, 'sample.txt'))
