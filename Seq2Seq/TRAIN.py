...
def train1(input_tensor, target_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion,
          max_length=MAX_LENGTH):
    encoder_hidden = encoder.initHidden()
...
def trainIters1(encoder, decoder, n_iters, print_every=1000, plot_every=1000, learning_rate=0.01):
    start = time.time()
    plot_losses = []
    print_loss_total = 0  # Reset every print_every
    plot_loss_total = 0  # Reset every plot_every
    ...
    for iter in range(1, n_iters + 1):
        ...
        loss = train1(input_tensor, target_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion)
...
if model==0:
    ...
    torch.save(encoder1, 'EncoderRNN.pt')
     # torch.save(encoder1.state_dict(), 'EncoderRNN.pt')
    # torch.save( EncoderRNN, 'EncoderRNN.pt')
    # torch.save(DecoderRNN, 'DecoderRNN.pt')
    # torch.save(DecoderRNN.state_dict(), 'DecoderRNN.pt')
    torch.save(attn_decoder1, 'AttnDecoderRNN.pt')
    # torch.save(attn_decoder1.state_dict(),'AttnDecoderRNN.pt')
    ...
    trainIters1(encoder1, attn_decoder1, 100000, print_every=5000)
    torch.save(encoder1, 'EncoderRNN01.pt')
    # torch.save(encoder1.state_dict(), 'EncoderRNN.pt')
    # torch.save( EncoderRNN, 'EncoderRNN.pt')
    # torch.save(DecoderRNN, 'DecoderRNN.pt')
    # torch.save(DecoderRNN.state_dict(), 'DecoderRNN.pt')
    torch.save(attn_decoder1, 'DecoderRNN01.pt')
    # torch.save(attn_decoder1.state_dict(),'AttnDecoderRNN.pt')
